                ...
                <div class="flex items-center">
                    <div class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary dark:bg-primary dark:text-white-light">
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M2 12.5C2 11.3954 2.89543 10.5 4 10.5H20C21.1046 10.5 22 11.3954 22 12.5V18C22 19.1046 21.1046 20 20 20H4C2.89543 20 2 19.1046 2 18V12.5Z"
                                stroke="currentColor"
                                stroke-width="2"
                            ...
                            <path
                                d="M2 12.5L2 6C2 4.89543 2.89543 4 4 4H20C21.1046 4 22 4.89543 22 6V12.5"
                                stroke="currentColor"
                                stroke-width="2"
                                stroke-linecap="round"
                                stroke-linejoin="round"
        ...
        <div class="panel mt-6">
            <div class="mb-5 flex flex-col gap-5 md:flex-row md:items-center">
                ...
                <div class="ltr:ml-auto rtl:mr-auto">
                    <input v-model="search" type="text" class="form-input w-auto" placeholder="ค้นหาบัญชี..." />
                </div>
            </div>

            <div class="datatables" v-if="!loading">
                <DataTable :items="filteredBankAccounts" :columns="columns" :pageSize="10" :showPagination="true" idKey="id">
                    ...
                    <template #is_active="{ row }">
                        ...
                        >
                            <span class="w-1.5 h-1.5 rounded-full mr-1" :class="row.is_active ? 'bg-green-500' : 'bg-red-500'"></span>
                    ...
                    <div class="p-5">
                        <form @submit.prevent="saveBankAccount">
                            <div class="mb-5">
                                ...
                                <CustomDropdown v-model="formData.bank_code" :options="bankOptions" placeholder="เลือกธนาคาร" />
                            </div>
                            <div class="mb-5">
...
<script lang="ts" setup>
    ...
    const fetchBankAccounts = async () => {
        loading.value = true;
        try {
            const { data, error } = await supabase.from('store_bank_accounts').select('*').order('created_at', { ascending: false });
    ...
    const filterBankAccounts = () => {
        if (!search.value) {
            ...
            filteredBankAccounts.value = bankAccounts.value.filter(
                (account) =>
    ...
    const saveBankAccount = async () => {
        ...
        if (!formData.value.bank_code) {
            toast.warning('กรุณาเลือกธนาคาร', 'กรุณาเลือกธนาคารจากรายการ');
            return;
        }
        if (!formData.value.bank_name.trim()) {
            toast.warning('กรุณากรอกชื่อธนาคาร', 'กรุณากรอกชื่อธนาคารให้ครบถ้วน');
            return;
        }
        if (!formData.value.account_number.trim()) {
            toast.warning('กรุณากรอกเลขที่บัญชี', 'กรุณากรอกเลขที่บัญชีให้ครบถ้วน');
            return;
        }
        if (!formData.value.account_name.trim()) {
            toast.warning('กรุณากรอกชื่อบัญชี', 'กรุณากรอกชื่อบัญชีให้ครบถ้วน');
            return;
        }
        ...
        try {
            if (isEdit.value) {
                ...
                if (error) {
                    ...
                    toast.error('แก้ไขบัญชีธนาคารไม่สำเร็จ', 'เกิดข้อผิดพลาดในการแก้ไขบัญชีธนาคาร กรุณาลองใหม่อีกครั้ง');
                    return;
                }
                ...
                toast.success('แก้ไขบัญชีธนาคารเรียบร้อยแล้ว', `แก้ไขข้อมูลบัญชี ${formData.value.account_number} สำเร็จ`);
            } else {
                ...
                if (error) {
                    ...
                    toast.error('เพิ่มบัญชีธนาคารไม่สำเร็จ', 'เกิดข้อผิดพลาดในการเพิ่มบัญชีธนาคาร กรุณาลองใหม่อีกครั้ง');
                    return;
                }
                ...
                toast.success('เพิ่มบัญชีธนาคารเรียบร้อยแล้ว', `เพิ่มบัญชี ${formData.value.account_number} สำเร็จ`);
            }
            ...
            toast.error('บันทึกข้อมูลไม่สำเร็จ', 'เกิดข้อผิดพลาดในการบันทึกข้อมูลบัญชีธนาคาร กรุณาลองใหม่อีกครั้ง');
        } finally {
            saving.value = false;
        }
    };
    ...
    const toggleBankAccountStatus = async (id: number) => {
        try {
            ...
            if (account) {
                ...
                const { error } = await supabase.from('store_bank_accounts').update({ is_active: newStatus }).eq('id', id);
