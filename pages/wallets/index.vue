        ...
        <div class="panel mt-6">
            <div class="mb-5 flex flex-col gap-5 md:flex-row md:items-center">
                ...
                <div class="ltr:ml-auto rtl:mr-auto">
                    <input v-model="search" type="text" class="form-input w-auto" placeholder="ค้นหากระเป๋า..." />
                </div>
            </div>

            <div class="datatables" v-if="!loading">
                <DataTable :items="filteredWallets" :columns="columns" :pageSize="10" :showPagination="true" idKey="id">
                    ...
                    <template #wallet_address="{ row }">
                        <code class="text-xs">
                            {{
                                row.wallet_address.length > 20 ? row.wallet_address.substr(0, 10) + '...' + row.wallet_address.substr(-10) : row.wallet_address
                            }}
                        </code>
                    </template>
...
<script lang="ts" setup>
    ...
    const fetchWallets = async () => {
        loading.value = true;
        try {
            const { data, error } = await supabase.from('store_wallets').select('*').order('created_at_th', { ascending: false });
    ...
    const filterWallets = () => {
        if (!search.value) {
            ...
            filteredWallets.value = wallets.value.filter(
                (wallet) =>
        ...
        try {
            if (isEdit.value) {
                ...
                if (error) {
                    ...
                    toast.error('แก้ไขกระเป๋าไม่สำเร็จ', 'เกิดข้อผิดพลาดในการแก้ไขกระเป๋า กรุณาลองใหม่อีกครั้ง');
                    return;
                }
                ...
                toast.success('แก้ไขกระเป๋าเรียบร้อยแล้ว', `แก้ไขข้อมูลกระเป๋า ${formData.value.wallet_name} สำเร็จ`);
            } else {
                // Create new wallet
                const { data, error } = await supabase
                    .from('store_wallets')
                ...
                if (error) {
                    ...
                    toast.error('เพิ่มกระเป๋าไม่สำเร็จ', 'เกิดข้อผิดพลาดในการเพิ่มกระเป๋า กรุณาลองใหม่อีกครั้ง');
                    return;
                }
                ...
                toast.success('เพิ่มกระเป๋าเรียบร้อยแล้ว', `เพิ่มกระเป๋า ${formData.value.wallet_name} สำเร็จ`);
            }
            ...
            toast.error('บันทึกข้อมูลไม่สำเร็จ', 'เกิดข้อผิดพลาดในการบันทึกข้อมูลกระเป๋า กรุณาลองใหม่อีกครั้ง');
        } finally {
            saving.value = false;
        }
    };
    ...
    const toggleWalletStatus = async (id: number) => {
        try {
            ...
            if (wallet) {
                ...
                const { error } = await supabase.from('store_wallets').update({ is_active: newStatus }).eq('id', id);
    ...
    const syncNetworkStatus = async (network?: string) => {
        try {
            ...
            const { data: activeWalletsByNetwork, error: walletsError } = await supabase.from('store_wallets').select('network').eq('is_active', true);
            ...
            if (activeNetworks.size > 0) {
                const { error: enableNetworksError } = await supabase.from('networks').update({ is_active: true }).in('id', Array.from(activeNetworks));
    ...
    const deleteWallet = async (id: number) => {
        ...
        if (confirmed) {
            try {
                ...
                const { error } = await supabase.from('store_wallets').delete().eq('id', id);
